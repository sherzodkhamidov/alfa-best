...
import Cateringback from "../../../assets/Cateringback.png"
import { useQuery } from 'react-query';
import loader from "../../../assets/loader1.svg"

function TransInfo({ lang }) {
  const { data, isLoading, isError, error } = useQuery(["cateringinfo"], () => {
    return axios
  ...
  if (isLoading) return <div className="loader1"><img src={loader} alt="" /></div>;
    const slicedData = data?.datas?.slice(2, 3);
    return (
      <div className="trans-all">
        <div className="transportBack">
          <img src={Cateringback} alt="rasm" />
        </div>
        <div>
          {slicedData.map((itm) => {
